...
The job of data wells is to enter data into the muonic DAQ chain. The data can either be acquired from hardware or pre-recorded data. 
Currently muonic4 provides these data wells:

#. A DAQ Server, which communicates with the DAQ card and takes live measurements
#. A reader from file, which reads pre-ecorded data from a file.
#. A reader from DB, which reads pre-recorded data from a mongoDB instance.

Data sinks
^^^^^^^^^^

The job of data sinks is to process data generated by a data well. **Data sinks are not exclusive, meaning multiple sinks can run simultaneously and process the same data**.
Currently muonic4 provides these data sinks:

#. A writer to a file, which saves incoming data to a file.
#. A writer to DB, which saves incoming data to a mongoDB instance.
#. A simple client, which simply dumps the incoming data to the terminal.
#. A Pulse Analyzer, which analyses and prints the pulse times.
#. A Rate Analyzer, which analyses the count rates.
